/*==================================================================================================
	Hyperion Engine
	Source/Renderer/DX12/DX12Core.cpp
	© 2021, Zachary Berry
==================================================================================================*/
...
namespace Hyperion
{
	...
	DX12Core::DX12Core()
		: m_OutputSurface( nullptr ), m_Resolution(), m_bVSync( true )
	{
	...
	bool DX12Core::Initialize( HWND inWindow, const ScreenResolution& inResolution, bool bVSync )
	{
		HYPERION_VERIFY( inWindow != nullptr, "[DX12] Window handle was null" );

		if( !InitializeResources( inWindow, inResolution, bVSync ) )
		{
			Console::WriteLine( "[ERROR] DX12: Failed to initialize core, resources couldnt be initialized" );
			return false;
		}

		m_bVSync			= bVSync;
		m_OutputSurface		= inWindow;
	...
	bool DX12Core::InitializeResources( HWND inWindow, const ScreenResolution& inRes, bool bVSync )
	{
		try
		{
			...
			m_Resolution.FullScreen		= inRes.FullScreen;
			...
			chainDesc.OutputWindow = m_OutputSurface;
	...
	void DX12Core::ShutdownResources()
	{
		m_RenderFence.Reset();
		m_CommandList.Reset();
	...
	bool DX12Core::UpdateResolution( const ScreenResolution& inRes )
	{
		return false;
	}


	bool DX12Core::UpdateVSync( bool bVSync )
	{
		return false;
	}


	bool DX12Core::IsResolutionValid( const ScreenResolution& inRes )
	{
		return false;
	}


	D3D12_CPU_DESCRIPTOR_HANDLE DX12Core::GetBackBufferTarget()
	{
		HYPERION_VERIFY( m_BackBufferTargetHeap != nullptr, "[DX12] Back buffer heap is null" );

		auto handle = m_BackBufferTargetHeap->GetCPUDescriptorHandleForHeapStart();
		if( m_BackBufferIndex == 1 )
		{
	...
	void DX12Core::FlipSwapChain()
	{
		...
		m_BackBufferIndex = m_BackBufferIndex > 0 ? 0 : 1;
	}
}