...
namespace Hyperion
{
	...
	public static class BC7Encoder
	{
		...
		public static bool Decode( byte[] inData, uint inWidth, uint inHeight, uint inPitch, bool bForceFullAlpha, out byte[] outData )
		{
			outData = null;

			if( ( inData?.Length ?? 0 ) < 4 || inWidth < 1 || inHeight < 1 || inPitch < 1 )
			{
				Core.WriteLine( "[Warning] BC7Encoder: Failed to decode BC7 image, invalid source data!" );
				return false;
			}

			// Call into c++ to perform decoding
			var result = BC7Library.Decode( inData, inWidth, inHeight, inPitch, true, out outData );
			if( result != BC7Result.Success )
			{
				Core.WriteLine( "[Warning] BC7Encoder: Failed to decode BC7 texture! Err#" + ( ( int ) result ).ToString() );
				outData = null;
				return false;
			}
			...
			if( bForceFullAlpha )
			{
				for( int i = 3; i < outData.Length; i += 4 )
				{
					outData[ i ] = ( byte ) 255;
				}
			}
