...
namespace Hyperion
{
	...
	public static class DXT1Encoder
	{
		...
		public static bool Decode( byte[] inData, uint inWidth, uint inHeight, uint inPitch, out byte[] outData )
		{
			...
			if( ( inData?.Length ?? 0 ) < 2 || inWidth < 1 || inHeight < 1 || inPitch < 1 )
			{
				Core.WriteLine( "[Warning] DXT1Encoder: Failed to decode DXT1 image, invalid source data!" );
				return false;
			}

			// Call into c++ to perform decoding
			var result = DXT1Library.Decode( inData, inWidth, inHeight, inPitch, out outData );
			if( result != DXT1Result.Success )
			{
				Core.WriteLine( "[Warning] DXT1Encoder: Failed to decode DXT1 texture! Err#" + ( ( int ) result ).ToString() );
				outData = null;
				return false;
			}
