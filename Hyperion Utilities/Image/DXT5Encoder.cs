...
namespace Hyperion
{
	...
	public static class DXT5Encoder
	{
		...
		public static bool Decode( byte[] inData, uint inWidth, uint inHeight, uint inPitch, out byte[] outData )
		{
			outData = null;

			if( ( inData?.Length ?? 0 ) < 4 || inWidth < 1 || inHeight < 1 || inPitch < 1 )
			{
				Core.WriteLine( "[Warning] DXT5Encoder: Failed to decode DXT5 image, invalid source data!" );
				return false;
			}

			// Call into c++ to perform decoding
			var result = DXT5Library.Decode( inData, inWidth, inHeight, inPitch, out outData );
			if( result != DXT5Result.Success )
			{
				Core.WriteLine( "[Warning] DXT5Encoder: Failed to decode DXT5 texture! Err#" + ( ( int ) result ).ToString() );
				outData = null;
				return false;
			}
