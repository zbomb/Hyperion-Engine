...
namespace Hyperion
{
	...
	*/
	...
	static class GrayscaleEncoder
	{
		...
		public static bool Decode( byte[] inData, uint inWidth, uint inHeight, bool bHasAlpha, out byte[] outData )
		{
			outData = null;

			// Check parameters
			if( ( inData?.Length ?? 0 ) < 1 || inWidth < 1 || inHeight < 1 )
			{
				Core.WriteLine( "[Warning] GrayscaleEncoder: Failed to decode grayscale image, source data invalid" );
				return false;
			}

			// Allocate output data
			outData = new byte[ inWidth * inHeight * 4 ];

			// Loop through source data
			int outOffset = 0;
			for( int offset = 0; offset < inData.Length; offset += ( bHasAlpha ? 2 : 1 ) )
			{
				// Read source pixel
				byte col = inData[ offset ];
				byte alpha = bHasAlpha ? inData[ offset + 1 ] : (byte)255;

				// Write output pixel
				Array.ConstrainedCopy( new byte[] { col, col, col, alpha }, 0, outData, outOffset * 4, 4 );
				outOffset++;
			}
